...
"""Generation script for the community model 'ecolicore2triple'."""
...
ecoli_model = cobra.io.read_sbml_model("./publication_runs/ecoli_models/sbml_models_in_used_form_with_demand_reactions/ecolicore2_loaded_and_saved_by_cobrapy_cleaned.xml")
...
for met in essential_in_metabolites+essential_out_metabolites:
    essential_metabolite_mapping[met] = (met+"\b").replace("_p\b", "").replace("_c\b", "")
...
community = Community(
    ...
    input_metabolite_ids=[(x+"\b").replace("_p\b", "").replace("_c\b", "") for x in essential_in_metabolites],
    output_metabolite_ids=[(x+"\b").replace("_p\b", "").replace("_c\b", "").replace("\b", "") for x in essential_out_metabolites+potential_product_metabolite_ids]
)
community_model = generate_community_model_with_no_growth(community, {"ecoli1": 1/3, "ecoli2": 1/3, "ecoli3": 1/3})

# Separate from commodelpy: Create dG0 dictionary for ecolicore2double
dG0_data_dict = json_load("./publication_runs/ecoli_models/reaction_id_to_dG0_mapping_jsons/reaction_id_to_dG0_mapping_for_ecolicore.json")
...
cobra.io.write_sbml_model(community_model, "./publication_runs/ecoli_models/publication_sbmls_and_dG0_jsons/ecolicore2triple_model.xml")
json_write("./publication_runs/ecoli_models/publication_sbmls_and_dG0_jsons/ecolicore2triple_dG0.json", dG0_data_dict)
