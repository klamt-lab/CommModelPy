...
# Internal modules
from ..commodepy import Community, SingleModel, generate_community_cobra_model
...
ecoli_model = cobra.io.read_sbml_model("./commodepy/publication_run/original_sbml_models_loaded_and_saved_by_cobrapy/ecolicore2_loaded_and_saved_by_cobrapy.xml")
...
# Save renamed ecolicore2 with cobrapy
# cobra.io.write_sbml_model(ecoli_model, "ecolicore2_loaded_and_saved_by_cobrapy.xml")

# Delete redundant biomass metabolite
ecoli_model.remove_metabolites([ecoli_model.metabolites.get_by_id("Biomass")])
...
# Triple it :DDD
ecoli_3 = copy.deepcopy(ecoli_1)
ecoli_3.species_abbreviation = "ecoli3"
...
community = Community(
    single_models=[ecoli_1, ecoli_2, ecoli_3],
...
# Separate from commodepy: Create dG0 dictionary for ecolicore2double
dG0_data_dict = json_load("./commodepy/publication_run/dG0_data_JSON/dG0.json")
...
for reaction in community_model.reactions:
    if reaction.id.startswith("EXCHG_"):
        ...
        is_essential = False
        for essential_metabolite in all_essential_metabolites:
            if reaction.id == ("EX_C_"+(essential_metabolite+"\b").replace("_e\b", "_exchg")):
                is_essential = True
                break
        if is_essential:
...
cobra.io.write_sbml_model(community_model, "./commodepy/publication_run/commodepy_publication_sbmls_and_dG0_jsons/ecolicore2triple__dG0_ARB_no_h2o_no_pi__metabolites_periplasmic_cytosolic.xml")

json_write("./commodepy/publication_run/commodepy_publication_sbmls_and_dG0_jsons/ecolicore2triple__dG0_ARB_no_h2o_no_pi__metabolites_periplasmic_cytosolic.json", dG0_data_dict)
