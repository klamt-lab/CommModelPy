...
commodepy. commodepy's source code is intersected in two parts:

1. The definition of all dataclasses which describe single models and a community.
2. The definition of the commodepy functions which can be used using the dataclasses.
...
class SingleModel:
    ...
    Use case
    ----------
    In commodepy, SingleModel instances are intended to be used
...
class Community:
    ...
    Use case
    ----------
    In commodepy, Community instances are directly used in order to create a
...
def community_model_fba_summary(model: cobra.Model, exchange_reaction_id_prefix: str = "EX_C_",
    ...
    """Performs an FBA (or any kind of modified optimization) on a commodepy-generated community model and prints the results.
    ...
    Arguments
    ----------
    * model: cobra.Model ~ The commodepy-generated RedCom community model as cobrapy Model instance.
...
def generate_community_cobra_model(community: Community) -> cobra.Model:
    """Creates a RedCom community cobrapy model from a commodepy Community instance.
...
def minimal_species_redcom_fba(model: cobra.Model, fixed_growth_rate: float,
    ...
    Arguments
    ----------
    * model: cobra.Model ~ The commodepy-generated community cobrapy model.
...
def split_reversible_organism_reactions(model: cobra.Model, organism_id_biomass_reaction_mapping: Dict[str, str]) -> cobra.Model:
    ...
    Arguments
    ----------
    * model: cobra.Model ~ A commodepy-generated community cobrapy model.
    * organism_id_biomass_reaction_mapping: Dict[str, str] ~ A dictionary with the community model's organism IDs
    ...
    for reaction in model.reactions:
        ...
        # Get biomass ID of the reaction using the commodepy standard naming scheme
        organism_id = reaction.id.split("_")[-1]
